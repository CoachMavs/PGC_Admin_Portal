<template>
  <h1 class="card-header pb-3">Export to Excel Zoom Meetings</h1>
  ...
  methods: {
    ...
    fetch(paramType = null) {
      ...
      let loadData = () => {
        ...
        axios({
          method: "get",
          url: process.env.VUE_APP_API + "ExportToPDF/fetchZoom",
          headers: {
    ...
    async exportToExcel() {
      ...
      try {
        ...
        // ① Add title row
        const title = `Zoom Meetings from ${this.formattedDateFrom} to ${this.formattedDateTo}`;
        ...
        anchor.href = url;
        anchor.download = "Zoom meetings.xlsx";
        anchor.click();
