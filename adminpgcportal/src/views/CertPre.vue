<template>
  <h1 class="card-header pb-3">Issued Pre-Inspection Certificate</h1>
  ...
  <v-dialog v-model="dialog" persistent width="700">
    <v-card>
      <v-card-title class="custom-title"> Add For Pre-Inspection </v-card-title>
      ...
      <v-card-text>
        <v-form ref="form" v-model="form">
          <v-container>
            ...
            <v-row>
              <v-col cols="12" md="5">
                <v-text-field ref="referenceNo" v-model="payloadExtra.ReferenceNo" label="Reference No." color="#14727a"
                  variant="outlined" hide-details="auto" readonly required :rules="[required]" validate-on="blur">
                  <template v-slot:append-inner>
                    <v-icon @click="dialogList = true" class="cursor-pointer">mdi-magnify</v-icon>
                  </template>
                </v-text-field>
              </v-col>
              <v-col cols="12" md="7">
            ...
            <v-row>
              ...
              <v-col cols="12" md="4">
                <v-text-field v-model="payloadExtra.ContactNo" label="Contact No." color="#14727a" variant="outlined"
            ...
            <v-row>
              <v-col cols="12">
                <v-textarea v-model="payloadExtra.ProblemsEncountred" label="Problems Encountered" color="#14727a"
                  variant="outlined" hide-details="auto" rows="1" readonly auto-grow />
              </v-col>
            </v-row>
            <v-row>
              <v-col cols="12" md="6">
...
export default {
  ...
  data: () => ({
    ...
    headers: [
      ...
      { title: "Problems Encountred", key: "ProblemsEncountred" },
      { title: "contactno", key: "contactno" },
  ...
  methods: {
    ...
    addPre() {
      ...
      axios({
        method: "post",
        url: process.env.VUE_APP_API + "PGCCert/addPRE",
        headers: {
    ...
    fetchUsers() {
      ...
      axios({
        method: "get",
        url: process.env.VUE_APP_API + "PGCCert/fetchRequest",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("xxx"),
        },
      })
    ...
    selectUser(item) {
      this.payload.repairlogID = item.ID;
      this.payloadExtra.ReferenceNo = item.ReferenceNo;
      this.payloadExtra.assignedto = item.AssignedTo;
      this.payloadExtra.Name_of_User = item.Name_of_User;
      this.payloadExtra.ContactNo = item.contactno;
    ...
    fetch(paramType = null) {
      ...
      let loadData = () => {
        ...
        axios({
          method: "get",
          url: process.env.VUE_APP_API + "PGCCert/fetchPre",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("xxx"),
          },
          params: myParameter,
    ...
    DeleteReq() {
      ...
      axios({
        method: "post",
        url: process.env.VUE_APP_API + "PGCCert/DeleteReq",
        headers: {
          Authorization: "Bearer " + localStorage.getItem("xxx"),
        },
        data: {
    ...
    OpenEditDialog(item) {
      this.payloadExtra.ReferenceNo = item.ReferenceNo1;
      this.payloadExtra.assignedto = item.AssignedTo;
      this.payloadExtra.Name_of_User = item.Name_of_User;
      this.payloadExtra.ContactNo = item.ContactNo;
      this.payloadExtra.Department = item.DeptDesc;
